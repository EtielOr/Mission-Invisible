...
Menu::Menu()
	:gameTitle(std::string("Mission Invisible"), ResourceManeger::getInstance().getFont())
{
	makeMainMenu();
	makePauseMenu();
	makeSureMenu();

	gameTitle.setCharacterSize(100);	// make the font size
	gameTitle.setFillColor(YELLOW_COLOR); // the color
...
bool Menu::askSure(std::string question)
{
	...
	questionText.setFont(ResourceManeger::getInstance().getFont()); // get font
...
void Menu::openMainMenu(sf::RenderWindow & window)
{
	...
	gameTitle.setPosition(30, 10);
	mainMenu[0]._text.setPosition(MAIN_SCREEN_SIZE / 3, gameTitle.getPosition().y + SPACE * 6); // set text Position
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(event))
		{
			...
			case sf::Event::Closed:
			{
				if (askSure("You want to exit the game?")) // case user press X
				{
					window.close();
					return;
				}
		...
		while (window.pollEvent(event))
		{
			...
			case sf::Event::KeyPressed:
				...
				case sf::Keyboard::Return: // case user press Enter
					...
					if (selectBotton == 1) // ask user if he press main screen
						if (askSure("You will lose all pogress are you sure?"))
						{
						...
						if (askSure("You want to exit the game?"))
						{
							CommandMap::getInstace().doCommand(pauseMenu[selectBotton]._cmd); // return the botton that press
							return;
						}
...
{
	_text.setFont(ResourceManeger::getInstance().getFont()); // get font
